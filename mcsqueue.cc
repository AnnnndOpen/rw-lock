...
void writer_queue::writer_lock(int thread_id)
{
  ...
  cs_time.fetch_add(1,memory_order_acquire); 
   if(pred != NULL)
  {
...
void writer_queue::writer_unlock(int thread_id)
{
  ...
  qnode* copy = cur_node;
  qnode* temp = nullptr;
  if(cur_node->next.load(memory_order_relaxed) == NULL) //acquire
  {
    
    if(atomic_compare_exchange_strong_explicit(&tail,&copy,temp,memory_order_release,memory_order_relaxed)) 
    {
      owner->set_read_flag(true);
      return;
    }
  ...
  cur_node->next.store(NULL,memory_order_relaxed);	 
  owner->set_read_flag(true);
}
