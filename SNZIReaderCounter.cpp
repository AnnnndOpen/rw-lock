...
void node::rootArrive()
	{
		...
		while (true)
		{
			...
			if (atomic_compare_exchange_strong_explicit(&xShared, &xLocal, xNew, memory_order_release, memory_order_relaxed))
			{
				break;
			}
			this_thread::yield();
		...
		if ((xNew & 0x80000000) == 0x80000000)
		{
			while (true)
			{
				...
				if (atomic_compare_exchange_strong_explicit(&owner->I, &Ilocal, Inew, memory_order_release, memory_order_relaxed))
				{
					break;
				}
				this_thread::yield();
			...
			xNew = 0x00000000 + v + c;
			atomic_compare_exchange_strong_explicit(&xShared, &xLocal, xNew, memory_order_release, memory_order_relaxed);
		}
	}
void node::rootDepart()
	{
		...
		while (true)
		{
			...
			if (atomic_compare_exchange_strong_explicit(&xShared, &xLocal, xNew, memory_order_seq_cst, memory_order_relaxed))
			{
				if (c >= 2)
					return;
				while (true)
				{
					Ilocal = owner->I.load(memory_order_relaxed);
					if ((xShared.load(memory_order_acquire) & 0x7fff0000) != v)
						return;
					Inew = 0x00000000 + (((Ilocal & 0x7fffffff) + 1) & 0x7fffffff);
					if (atomic_compare_exchange_strong_explicit(&owner->I, &Ilocal, Inew, memory_order_seq_cst, memory_order_relaxed))
					{
						return;
					}
					this_thread::yield();
...
void node::arrive()
	{
		...
		while (!succ)
		{
			...
			{
				xNew = half + v + c + 1;
				if (atomic_compare_exchange_strong_explicit(&xShared, &xLocaltmp, xNew, memory_order_release, memory_order_relaxed))
				{
					succ = true;
				}
				else
